...
class Circle(Shape):
    def __init__(self, id_tag, center, radius, ratio = 2.5, phys_tag = None,\
        ...
        if self.phys_tag:
            ...
            self.physical_surfaces.append(self.square.physical_surface)
            for surface in self.surfaces:
                self.physical_surfaces.append( Physical_Surface(surface.id_tag, surface, self.transfinite))
    ...
    def __str__(self):
        ...
        if self.phys_tag:
            for phys_surf in self.physical_surfaces:
                circle_string = circle_string + phys_surf.__str__()
            if self.recombine:
...
class Physical_Surface():
    ...
    """        
    def __init__(self, id_tag, plane_surface):
        self.id_tag = id_tag
        self.plane_surface = plane_surface
    def __str__(self):
        return 'Physical Surface(%s) = {%s};\n' %(self.id_tag, self.plane_surface.id_tag)        
